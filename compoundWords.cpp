...
bool
Trie :: isCompoundWord(string word, int word_length) {
        TrieNode *r = root;
        int cnt = 0;
        if (word.empty())
                return true;
 ...
 *  It displays the first and second longest compound word, with the total
 *  number of compound words in the given file.
 */
void
findCompoundWords(Trie &T, char *filename) {
        ...
        file.close();
        cout << "1st Longest word: " << setw(30) << longest_word << " Length: " << max_length << endl;
        cout << "2nd Longest word: " << setw(30) << second_longest_word << " Length: " << second_max_length << endl;
        cout << "Total compound words: " << c << endl;
}
